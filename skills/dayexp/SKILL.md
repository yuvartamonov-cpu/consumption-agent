...
```bash
python3 <skill-dir>/scripts/dayexp_report.py
```

Скрипт:
1. Запускает `daily_cheque_scan.py` из `consumption_agent/` — сканирование всех почт + SMS
2. Читает из БД `consumption.db` все записи за `date('now')`
3. Выводит отчёт в формате:
   - Заголовок: «📊 Расходы за ДД.ММ.ГГГГ»
...
Команда `/dayexp` в `telegram_bot.py`:
- Отвечает «🔍 Сканирую почты и SMS за сегодня...»
