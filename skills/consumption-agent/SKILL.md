...
## Структура кода (обновлено 18.05.2026)

```
consumption_agent/
├── telegram_bot.py            # 1 274 строк, в основном entrypoint + photo_handler (≈543)
├── bot/                       # Telegram-слой
...
│       └── photos.py          # ⚠️ placeholder — photo_handler пока в telegram_bot.py
├── services/
...
Остаточные прямые `sqlite3.connect` (план — убрать к 21.05): `ml_source_matcher.py`, `ml_search_v2.py`, `daily_cheque_scan.py`, `scripts/fines_bot.py`, `sms_monitor.py`.
