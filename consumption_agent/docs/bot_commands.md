...
Текстовый поиск по Memory Lane (SQL LIKE, без эмбеддингов). Ищет по полям caption, name, description, brand и style_tags. Регистронезависимо, в т.ч. для кириллицы.

Опциональные флаги:
- `--topic <тема>` — только записи с этой темой
- `--brand <бренд>` — фильтр по бренду
- `--color <цвет>` — цвет (ищется внутри style_tags / описания)

Рядом с каждым результатом — кнопка «🔍 #id», запускающая `/ml_search` для этой записи.

```
/ml_find пальто
/ml_find пальто --color чёрный
/ml_find --topic обувь --brand Nike
```

### `/ml_profile [тема]`

Профиль вкуса по агрегации Memory Lane (без LLM). Считает частоты и выводит топ-5: что нравится / не нравится, бренды, цвета, материалы, стиль-теги, плюс последние 5 примеров. Без аргумента — по всей базе; с темой — только по ней.
...
CTR по источникам + bandit snapshot + source matcher (tier × geo × CTR) + последние 8 событий (impressions/clicks).

Используется для:
- Понимать, какие маркетплейсы дают клики
- Видеть, как bandit обновляется (Beta α/β по категориям)
- Оценивать качество новой системы выбора источников (`ml_source_matcher`): CTR по уровням источника (manufacturer/distributor/aggregator/marketplace) и гео
